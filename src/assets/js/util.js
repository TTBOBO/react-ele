...
let utils = {
	...
	getAccessRouter(routers) {
		...
		routers.forEach((item, index) => {
			if (item.access) {
				if (this.showRouter(item.access, access) || this.getSessionStorage('role_id') == "*") {
					if (item.children && item.children.length == 1) {
						...
						menuList[len - 1].children = childrenArr;  //设置当前 item 有权限的路由
				...
				if (!item.children) {
					...
					if (childrenArr === undefined || childrenArr.length === 0) {
						menuList.splice(len - 1, 1);  //不存在时 删除当前元素
					} else {
						let handledItem = JSON.parse(JSON.stringify(menuList[len - 1]));
						handledItem.children = childrenArr;
						menuList.splice(len - 1, 1, handledItem);  //删除当前并替换
					}
				}
			}
	...
	showRouter(access, accessArr) {
		...
		return this.getIndexOf(access, accessArr);
	},
	 ...
	 * @param {*} pathname  当前理由
	 */
	getOpenMenu(menuList,pathname){
		let path = [],firstItem = false,firstindex = null;
				...
				if(item.path == pathname){
					...
					if(item.children){
						item.children.forEach ((_item,_index) =>{
							if(_item.path == pathname){
								path.push(_item.path);
							}
						})
					}
				}
